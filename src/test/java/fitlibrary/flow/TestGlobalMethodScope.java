...
public class TestGlobalMethodScope {
	...
	public void regularAllowing() {
		...
		context.checking(new Expectations() {{
			allowing(runtimeContext).getDynamicVariables(); will(returnValue(dynamicVariables));
		}});
	}
	...
	public void getBecomesTimeout() {
		context.checking(new Expectations() {{
			oneOf(runtimeContext).getTimeout(GlobalActionScope.BECOMES_TIMEOUT, 1000); will(returnValue(1000));
		}});
	...
	public void getTimeout() {
		...
		context.checking(new Expectations() {{
			oneOf(runtimeContext).getTimeout(var, 1000); will(returnValue(1000));
		}});
	...
	public void dynamicVariableDoesNotExist() {
		...
		context.checking(new Expectations() {{
			oneOf(dynamicVariables).get(var); will(returnValue(null));
		}});
	...
	public void dynamicVariableExists() {
		final String var = "exists";
		context.checking(new Expectations() {{
			oneOf(dynamicVariables).get(var); will(returnValue("a"));
		}});
	...
	public void theStopWatchNotStarted() {
		context.checking(new Expectations() {{
			oneOf(dynamicVariables).get(GlobalActionScope.STOP_WATCH); will(returnValue(null));
		}});
	...
	public void startAndstopStopWatch() {
		context.checking(new Expectations() {{
			...
			oneOf(dynamicVariables).get(GlobalActionScope.STOP_WATCH); will(returnValue(new StopWatch()));
		}});
